...
const searchCustomer = async ({ filter, page = 1, per_page = 10, sortBy = {} }) => {
...
const Customer = () => {
  ...
  const title = f({ id: 'customer.title' });
  const description = '';
  ...
  const role = roleResources?.find((item) => item.resource_name.toLowerCase() === 'customer');

  const columns = useMemo(() => {
    return [
      {
        Header: f({ id: 'customer.field.no' }),
        accessor: 'id',
        sortable: false,
      ...
      {
        Header: f({ id: 'customer.field.phone' }),
        accessor: 'phone',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => (
          <div className="text-medium" style={{ width: '6.5rem' }}>
            {cell.value || '-'}
          </div>
        ),
      },
      {
        Header: f({ id: 'customer.field.level' }),
        accessor: 'level_name',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => (
      ...
      {
        Header: f({ id: 'customer.field.rate' }),
        accessor: 'level_rate',
        sortable: false,
        headerClassName: 'text-medium text-muted-re text-center',
        Cell: ({ cell }) => (
          <div className="text-medium text-end" style={{ width: '4rem' }}>
            {useConvertCurrency(cell.row?.original?.level_rate, 2) || '-'}
          </div>
        ),
      },
      {
        Header: f({ id: 'customer.field.verify' }),
        accessor: 'verify',
        sortable: false,
        headerClassName: 'text-medium text-muted-re text-center',
        Cell: ({ row }) => {
      ...
      {
        Header: f({ id: 'customer.field.createAt' }),
        accessor: 'created_at',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => (
      ...
      {
        Header: f({ id: 'customer.field.updateAt' }),
        accessor: 'updated_at',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => (
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="text-medium d-flex flex-row gap-1 icon-hover" style={{ width: '3rem' }}>
            <div>
              <NavLink
                to={`${role?.can_view === 0 ? `/setting/customer` : `/setting/customer/${row.values.id}?view`}`}
            ...
            <div>
              {role?.can_update === 0 ? (
                ...
                <NavLink
                  to={`/setting/customer/${row.values.id}?edit`}
                  className=" text-truncate h-100 d-flex align-items-center"
  ...
  const { isFetching, refetch } = useQuery(
    ...
    () => searchCustomer({ filter, per_page: pageSize, page, sortBy: sortByFromTable(sortBy) }), // ใช้ฟังก์ชันนี้แทนการเรียกโดยตรง
    {
  ...
  const getCustomerLevel = async () => {
    const res = await request({ url: `/customer_level`, method: 'GET' });
  ...
  return (
    <>
      ...
      <PageTitle
        ...
        addButton={{ label: f({ id: 'customer.field.add' }), link: '/setting/customer/new' }}
