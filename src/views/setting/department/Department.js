...
const searchDepartment = async ({ filter, page = 1, per_page = 10, sortBy = {} }) => {
...
const Department = () => {
  ...
  const title = f({ id: 'department.title' });
  const description = '';
  ...
  const role = roleResources?.find((item) => item.resource_name.toLowerCase() === 'department');
  const columns = useMemo(() => {
    return [
      {
        Header: f({ id: 'department.field.no' }),
        accessor: 'id',
        sortable: false,
      ...
      {
        Header: f({ id: 'department.field.name' }),
        accessor: 'name',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => {
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="text-medium d-flex flex-row gap-1 icon-hover" style={{ width: '3rem' }}>
            <div>
              <NavLink
                to={`${role?.can_view === 0 ? `/setting/department` : `/setting/department/${row.values.id}?view`}`}
            ...
            <div>
              {role?.can_update === 0 ? (
                ...
                <NavLink
                  to={`/setting/department/${row.values.id}?edit`}
                  className=" text-truncate h-100 d-flex align-items-center"
  ...
  const { isFetching, refetch } = useQuery(
    ...
    () => searchDepartment({ filter, per_page: pageSize, page, sortBy: sortByFromTable(sortBy) }), // ใช้ฟังก์ชันนี้แทนการเรียกโดยตรง
    {
  ...
  const handleConfirmDelete = async () => {
    await request({ url: `/department/${getId}`, method: 'DELETE' });
    refetch();
    setIsDeleting(false);
  };
  ...
  return (
    <>
      ...
      <PageTitle
        ...
        addButton={{ label: f({ id: 'customer.field.add' }), link: '/setting/department/new' }}
