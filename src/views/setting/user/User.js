...
const searchUser = async ({ filter, page = 1, per_page = 10, sortBy = {} }) => {
...
const User = () => {
  ...
  const role = roleResources?.find((item) => item.resource_name.toLowerCase() === 'user');
  const columns = useMemo(() => {
    return [
      {
        Header: f({ id: 'user.field.no' }),
        accessor: 'id',
        sortable: false,
      ...
      {
        Header: f({ id: 'user.field.department' }),
        accessor: 'department_data',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => {
          return (
      ...
      {
        Header: f({ id: 'department.field.status' }),
        accessor: 'active',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ row }) => {
          return (
            <div className="text-medium text-center" style={{ width: '5rem' }}>
              {row.original.active ? (
      ...
      {
        Header: f({ id: 'department.field.createAt' }),
        accessor: 'created_at',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => (
      ...
      {
        Header: f({ id: 'department.field.updateAt' }),
        accessor: 'updated_at',
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => (
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <div className="text-medium d-flex flex-row gap-1 icon-hover" style={{ width: '3rem' }}>
            <div>
              <NavLink
                to={`${role?.can_view === 0 ? `/setting/user` : `/setting/user/${row.values.id}?view`}`}
            ...
            <div>
              {role?.can_update === 0 ? (
                ...
                <NavLink to={`/setting/user/${row.values.id}?edit`} className=" text-truncate h-100 d-flex align-items-center" style={{ cursor: 'pointer' }}>
                  <img src="/img/icons/edit.png" alt="edit" />
                </NavLink>
              )}
            </div>
  ...
  const { isFetching, refetch } = useQuery(
    ['searchUser', filter, pageSize, sortBy, page],
    () => searchUser({ filter, per_page: pageSize, page, sortBy: sortByFromTable(sortBy) }), // ใช้ฟังก์ชันนี้แทนการเรียกโดยตรง
    {
  ...
  const handleConfirmDelete = async () => {
    await request({ url: `/users/${getId}`, method: 'DELETE' });
    refetch();
    setIsDeleting(false);
  };

  const getDepartment = async () => {
    const res = await request({ url: `/department`, method: 'GET' });
  ...
  return (
    <>
      ...
      <PageTitle
        ...
        addButton={{ label: f({ id: 'user.field.add' }), link: '/setting/user/new' }}
